...
The synthetic e-commerce data is generated by [factorhouse/thelook-ecomm](https://github.com/factorhouse/examples/tree/main/datagen/thelook-ecomm), a real-time data simulator forked and customized for this lakehouse. It is based on the original **TheLook eCommerce** dataset from Looker (available on Google Cloud Marketplace), re-engineered to support continuous CDC streaming instead of static batch loads.

The generator simulates realistic e-commerce activity — user registrations, order placements, order item additions, session events (page views, cart actions, purchases), and order status updates — directly into PostgreSQL, triggering CDC events downstream via Debezium.
...
│           ├── intermediate/   
│           └── mart/           
├── docker-compose.yaml         # Service container
